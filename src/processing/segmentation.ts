...
const fitPlanes = (samples: Sample[]) => {
  ...
  return targets.map((target) =>
    solve3x3(normal.map((row) => [...row]), target),
...
const fitBackgroundModel = (
  ...
  const step = Math.max(1, Math.floor(Math.min(bounds.width, bounds.height) / 96))
...
const selectCoreColorClusters = (
  ...
  const corePixels = ranked.reduce((total, [, cluster]) => total + cluster.count, 0)
...
const textColorFromMask = (
  ...
  const strongest = pixels.slice(0, Math.max(1, Math.ceil(pixels.length * 0.45)))
...
export const segmentGlyphs = (
  ...
  const candidateMargin = Math.max(2, Math.round(detection.bounds.height * 0.28))
  ...
  for (let y = 0; y < bounds.height; y += 1) {
    for (let x = 0; x < bounds.width; x += 1) {
      ...
      const insideCandidate =
        ...
        globalX <= detection.bounds.x + detection.bounds.width + candidateMargin &&
        globalY >= detection.bounds.y - candidateMargin &&
        globalY <= detection.bounds.y + detection.bounds.height + candidateMargin
      const local = y * bounds.width + x
  ...
  core = morph(morph(core, bounds.width, bounds.height, 1, 'dilate'), bounds.width, bounds.height, 1, 'erode')
  ...
  for (let index = 0; index < size; index += 1) {
    ...
    if (removalMask[index]) {
      ...
      softMask[index] = Math.round(255 * clamp(0.82 + normalized * 0.18, 0.82, 1))
    }
  }
